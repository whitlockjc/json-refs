...
|  | <code>[ResolvedRefsResults](#module_json-refs..ResolvedRefsResults)</code> | An object whose keys are JSON Pointers *(fragment version)* to where the JSON Reference is defined and whose values are [ResolvedRefDetails](#module_json-refs..ResolvedRefDetails) |
| value | <code>object</code> | The retrieved document |
